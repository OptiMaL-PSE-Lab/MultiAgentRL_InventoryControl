...
"""
This environment is for a multi product, multi echelon supply chain where action 
space is defined as continuos order replenishment.
...
class MultiAgentInvManagementDiv(MultiAgentEnv):
    def __init__(self, config, **kwargs):
        ...
        self.delay = config.get("delay", np.array([1,2,1,1,1], dtype = np.int8))
        ...
        self.noise_delay_threshold = config.get("noise_delay_thresholdd", 0.1)
    ...
    def reset(self, customer_demand=None, noisy_delay=True, noisy_delay_threshold=0.1, \
        ...
        # Assign customer demand to each retailer and product th
        ...
        for i in range(self.num_nodes):
            ...
            for product in range(self.num_products):
                self.backlog_to[i][product] = 0 
        print("backlog dict", self.backlog_to)
            ...
            if self.prev_actions:
                ...
                for j in range(self.prev_length):
                    if j < t:
                        ...
                        print(self.order_r)
                        #print("order r", self.order_r[t-1-j, node, product])
    ...
    def step(self, action_dict):
        ...
        for echelon in range(len(self.echelons)):
            for node in range(len(self.network)):
                for product in range(self.num_products):
                    ...
                    self.order_r[t, node, product] = \
                    self.rev_scale(action_dict[node_name], 0, self.order_max[node, product], self.a, self.b)
                
                    self.order_r[t, node, product] = np.round(self.order_r[t, node, product], 0).astype(int)

                    """for i in range(self.num_nodes * self.num_products):
                        product = i % p 
                        node = i // p
        ...
        for product in range(self.num_products):
            self.ship[t, :, product] = np.minimum(
            self.backlog[t, :, product] + self.demand[t, :, product], 
            self.inv[t, :, product] + self.acquisition[t, :, product])
                        ...
                        if self.backlog[t, i, product] > 0:
                            ...
                            and ship_amount > 0:
                                ...
                                for node in self.connections[i]:
                                    ...
                                    if self.backlog_to[node][product] > 0:
                                        self.ship_to_list[t][node][product] += 1  
                                        # increase amount shipped to node
                                        self.backlog_to[node][product] -= 1  
                            ...
                            if ship_amount > 0 and self.demand[t, i, product] > 0:
                                ...
                                for node in self.connections[i]:
                                    for product in range(self.num_products):
                                        self.backlog_to[node][product] += outstanding_order[node][product]
                            ...
                            while ship_amount > 0:
                                for node in self.connections[i]:
                                    for product in range(self.num_products):
                                        ...
                                        self.order_r[t, node, product] + self.backlog_to[node][product]:
    ...
    def update_acquisition(self):
        ...
        m = self.num_nodes
        t = self.period
        self.noise_delay = self.noise_delay
        self.noise_delay_threshold = self.noise_delay_threshold
...
print(type(test_env.observation_space))
    ...
    def plots(self):
        #plt.plot(self.inv , self.period)
        # assume self.inv is already initialized
        periods = self.inv.shape[0] 
        num_nodes = self.inv.shape[1]

        # loop over columns and plot inv vs periods
        for i in range(num_nodes):
            plt.plot(range(periods), self.inv[:,i], label='Node {}'.format(i))
...
'''
using this to test the code of the environment to see if it runs 
config = {
}
x = MultiAgentInvManagementDiv(config=config)
'''