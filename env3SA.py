...
"""
This single agent environment is for a multi product, multi echelon supply chain where action 
space is defined as (s,S) - the parameters of an inventory control policy.
"""
...
class InvManagementDiv(gym.Env):
    def __init__(self, config):
        ...
        for i in range(self.num_nodes):
            self.node_price[i] = 2.5 * stage_price[get_stage(i, self.network)]
        ...
        self.backlog_cost = config.get("backlog_cost", np.ones(self.num_nodes)*2.5)
        ...
        if self.standardise_actions:
            self.action_space = gym.spaces.Box(
                low=np.ones(2*self.num_nodes, dtype=np.float64)*self.a,
                high=np.ones(2*self.num_nodes, dtype=np.float64)*self.b,
                dtype=np.float64,
                shape=(2*self.num_nodes,)
            )
        else:
            self.action_space = gym.spaces.Box(
                low=np.zeros(2*self.num_nodes, dtype=np.int32),
                high=np.int32(2*self.order_max),
                dtype=np.int32,
                shape=(2*self.num_nodes,)
            )
    ...
    def reset(self, customer_demand=None, noisy_delay=False, noisy_delay_threshold=0):
        ...
        if customer_demand is not None:
            ...
            if self.demand_dist == "custom":
                self.customer_demand = self.config.get("customer_demand", np.ones((len(self.retailers), self.num_periods), dtype=np.int16) * 5)
    ...
    def step(self, action):
        ...
        action = np.squeeze(action).reshape(-1, 2)
        ...
        if self.standardise_actions: 
            #print(s1, s2, self.order_max[0])
            s1 = self.rev_scale(s1, 0, self.order_max[0], self.a, self.b)
            s2 = self.rev_scale(s2, 0, self.order_max[0], self.a, self.b)
        for node in range(self.num_nodes):
            if s2[node] < s1[node]:
                s2[node] = s1[node]

        for node in range(self.num_nodes):
            if self.inv[t, node] < s1[node]:
                #print("order_r0",np.maximum(s2[node] - self.inv[t, node], 0))
                self.order_r[t, node] = np.round(np.minimum(np.maximum(s2[node] - self.inv[t, node], 0), self.order_max[node]), 0).astype(int)
            else:
                self.order_r[t, node] = 0

        #print("order_r", self.order_r[t, :])
